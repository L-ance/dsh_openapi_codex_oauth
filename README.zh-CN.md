...
```sh
npx --yes --package=https://github.com/L-ance/dsh_openapi_codex_oauth/releases/download/v0.2.3/dsh-openapi-codex-oauth-0.2.3.tgz dsh-openapi-codex-oauth install --profile web
```

安装器会把当前 Release 和适用于当前系统的 Codex 原生包整理成稳定的本地 tarball，再注册到 DSH。它固定使用已验证的 `@deepseek-ai/dsh@0.1.1-rc.2`；如果 profile 已经存在，还会读取 `node_modules/.modules.yaml` 并使用创建该 profile 的 pnpm 版本，从而避免 `ERR_PNPM_UNEXPECTED_STORE` 和远程 tarball integrity 缺失。
...
`--local` 会打包当前源码构建并安装到 DSH profile，适合开发和调试。安装或更新后需要重启正在运行的 DSH 进程。
...
```sh
npx --yes --package=https://github.com/L-ance/dsh_openapi_codex_oauth/releases/download/v0.2.3/dsh-openapi-codex-oauth-0.2.3.tgz dsh-codex-login
```

无浏览器或远程环境可以使用设备码：

```sh
npx --yes --package=https://github.com/L-ance/dsh_openapi_codex_oauth/releases/download/v0.2.3/dsh-openapi-codex-oauth-0.2.3.tgz dsh-codex-login --device-auth
```
...
```sh
npx --yes --omit=optional --package=https://github.com/L-ance/dsh_openapi_codex_oauth/releases/download/v0.2.3/dsh-openapi-codex-oauth-0.2.3.tgz dsh-openapi-codex-oauth uninstall --profile web
npx --yes --package=https://github.com/L-ance/dsh_openapi_codex_oauth/releases/download/v0.2.3/dsh-openapi-codex-oauth-0.2.3.tgz dsh-openapi-codex-oauth uninstall --profile web --purge-auth
```
